...
const INSIDE_STRING = {
  'bash': COMMAND_AFTER_HEREDOC,
  'environment': {
    pattern: RegExp('\\$' + ENV_VARS),
    alias: 'constant',
  },
  'variable': [
...
const bashGrammarObject = {
  ...
  'variable': INSIDE_STRING.variable,
  'function': {
...
COMMAND_AFTER_HEREDOC.inside = bashGrammarObject
...
const inside = INSIDE_STRING.variable[1].inside

let toBeCopiedIndex = 0
while (toBeCopiedIndex < TO_BE_COPIED.length) {
  inside[TO_BE_COPIED[toBeCopiedIndex]] = bashGrammarObject[TO_BE_COPIED[toBeCopiedIndex]]
  toBeCopiedIndex += 1
}

export const bash = new Grammar('bash', bashGrammarObject)
